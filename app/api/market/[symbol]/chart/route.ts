...
function generateChartData(symbol: string, interval: string, limit: number): ChartDataPoint[] {
  const data: ChartDataPoint[] = [];
  const now = Date.now();

  // Base price for each symbol
  const basePrices: { [key: string]: number } = { BTC: 42500, ETH: 2245, USDC: 1, LINK: 18.45, UNI: 8.32 };
  ...
  for (let i = limit - 1;i >= 0;i--) {
    ...
    // Carry the closing price forward so the next candle starts from it
    basePrice = close;
  }
...
export async function GET(request: NextRequest, { params }: { params: { symbol: string } }) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Failed to fetch chart data' }, { status: 500 });
  }
}

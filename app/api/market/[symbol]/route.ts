...
const mockDetailedData: { [key: string]: DetailedMarketData } = {
  BTC: {
    ...
    volatility: 3.2,
    beta: 1.0,
    correlations: { ETH: 0.82, LINK: 0.65, UNI: 0.71 },
  ...
  ETH: {
    ...
    volatility: 3.8,
    beta: 1.15,
    correlations: { BTC: 0.82, LINK: 0.72, UNI: 0.85 },
    technicalIndicators: { rsi: 58.3, macd: 42.1, movingAverage50: 2180.0, movingAverage200: 2050.0 },
    lastUpdated: new Date().toISOString()
  }
};
...
export async function GET(request: NextRequest, { params }: { params: { symbol: string } }) {
  try {
    ...
    return NextResponse.json({ success: true, data: { ...marketData, lastUpdated: new Date().toISOString() } });
  } catch (error: any) {

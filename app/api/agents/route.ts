...
export interface Agent {
  ...
  metrics: { totalExecutions: number, successRate: number, avgExecutionTime: number, uptime: number };
  tags: string[];
  subscribers: number;
  verified: boolean;
  createdAt: string;
...
const mockAgents: Agent[] = [{
  ...
  type: 'trading',
  status: 'active',
  capabilities: ['MEV Protection', 'Flash Loan Integration', 'Multi-DEX Support', 'Gas Optimization'],
  ...
  type: 'automation',
  status: 'active',
  capabilities: ['Risk Assessment', 'Auto-Rebalancing', 'Tax Optimization', 'Multi-Asset Support'],
  pricing: { type: 'flat', amount: 50, currency: 'USDC', billingPeriod: 'monthly' },
  metrics: { totalExecutions: 8932, successRate: 98.2, avgExecutionTime: 15.7, uptime: 99.9 },
  ...
  type: 'analytics',
  status: 'active',
  capabilities: ['Social Media Analysis', 'On-chain Analytics', 'News Aggregation', 'Sentiment Scoring'],
  pricing: { type: 'flat', amount: 100, currency: 'USDC', billingPeriod: 'monthly' },
  metrics: { totalExecutions: 45678, successRate: 91.3, avgExecutionTime: 5.2, uptime: 99.5 },
  ...
  type: 'monitoring',
  status: 'active',
  capabilities: ['Whale Detection', 'Real-time Alerts', 'Pattern Recognition', 'Multi-chain Support'],
  pricing: { type: 'flat', amount: 75, currency: 'USDC', billingPeriod: 'monthly' },
  metrics: { totalExecutions: 32145, successRate: 96.8, avgExecutionTime: 1.8, uptime: 99.7 },
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (minSuccessRate) {
      filteredAgents = filteredAgents.filter((a) => a.metrics.successRate >= parseFloat(minSuccessRate));
    }
    ...
    return NextResponse.json({ success: true, data: paginatedAgents, meta: { total, limit, offset, hasMore: offset + limit < total } });
  } catch (error: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newAgent: Agent = {
      ...
      pricing: body.pricing || { type: 'free', amount: 0, currency: 'USDC' },
      metrics: { totalExecutions: 0, successRate: 0, avgExecutionTime: 0, uptime: 0 },
      tags: body.tags || [],
      subscribers: 0,
      verified: false,
    ...
    return NextResponse.json({ success: false, error: error.message || 'Failed to deploy agent' }, { status: 500 });
  }
}

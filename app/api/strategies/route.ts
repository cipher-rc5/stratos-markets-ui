...
const mockStrategies: Strategy[] = [{
  ...
  creator: '0x5678...',
  version: '2.1.5',
  performance: { roi: 18.2, sharpeRatio: 2.1, maxDrawdown: -8.7, winRate: 72.3, totalTrades: 527 },
  ...
  creator: '0x9abc...',
  version: '3.0.2',
  performance: { roi: 42.8, sharpeRatio: 1.5, maxDrawdown: -18.9, winRate: 61.2, totalTrades: 891 },
  ...
  creator: '0xdef0...',
  version: '1.8.3',
  performance: { roi: 15.6, sharpeRatio: 2.3, maxDrawdown: -6.2, winRate: 75.8, totalTrades: 445 },
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (riskLevel) {
      filteredStrategies = filteredStrategies.filter((s) => s.riskLevel === riskLevel);
    }

    if (minRoi) {
      filteredStrategies = filteredStrategies.filter((s) => s.performance.roi >= parseFloat(minRoi));
    }

    if (maxRoi) {
      filteredStrategies = filteredStrategies.filter((s) => s.performance.roi <= parseFloat(maxRoi));
    }
    ...
    return NextResponse.json({ success: true, data: paginatedStrategies, meta: { total, limit, offset, hasMore: offset + limit < total } });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message || 'Failed to fetch strategies' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newStrategy: Strategy = {
      ...
      performance: body.performance || { roi: 0, sharpeRatio: 0, maxDrawdown: 0 },
    ...
    return NextResponse.json({ success: true, data: newStrategy }, { status: 201 });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message || 'Failed to create strategy' }, { status: 500 });
  }
}

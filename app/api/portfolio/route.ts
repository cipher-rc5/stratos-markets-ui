...
export async function GET(request: NextRequest) {
  try {
    ...
    const portfolio = { ...mockPortfolio, walletAddress, lastUpdated: new Date().toISOString() };

    return NextResponse.json({ success: true, data: portfolio });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message || 'Failed to fetch portfolio' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Failed to create/update portfolio' }, { status: 500 });
  }
}

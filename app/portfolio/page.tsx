...
const PortfolioPage = () => {
  ...
  const tokenData: TokenItem[] = useMemo(() => {
    const mapped = balances
      .filter((balance) => balance.value_usd > 0 && !defiTokenAddresses.has(balance.address.toLowerCase()))
      .map((balance) => {
  ...
  const totalValue = useMemo(
    () =>
      ...
      protocolData.reduce((acc, protocol) => acc + (protocol.liquidity ?? 0), 0),
  ...
  return (
    <div
      className={`min-h-screen bg-black text-white selection:bg-[#ccff00] selection:text-black p-6 md:p-12 ${bodyFontClass}`}
    >
      <Navbar />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gradient-to-br from-gray-900/50 to-black border border-gray-800 p-6 hover:border-[#ccff00]/30 transition-all group">
            <div className="flex items-start justify-between mb-4">
              <div className={`text-xs text-gray-500 uppercase tracking-wider font-bold ${headingFontClass}`}>
                Net Worth
              </div>
            ...
            <div className="text-3xl font-bold text-white mb-3 tracking-tight">
              $
              {(performanceData.totalValue + performanceData.nftValue).toLocaleString(undefined, {
          ...
          <div className="bg-gradient-to-br from-gray-900/50 to-black border border-gray-800 p-6 hover:border-[#ccff00]/30 transition-all group">
            <div className="flex items-start justify-between mb-4">
              <div className={`text-xs text-gray-500 uppercase tracking-wider font-bold mb-2 ${headingFontClass}`}>
                NFT Value
              </div>
              <ImageIcon className="w-5 h-5 text-gray-700 group-hover:text-[#ccff00] transition-colors" />
            </div>
            <div className="text-3xl font-bold text-white mb-3 tracking-tight">
              ${performanceData.nftValue.toLocaleString()}
            </div>
            <div className="flex items-center gap-2 text-sm">
                    ...
                    <tr key={idx} className="hover:bg-gray-900/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <div className={`text-sm text-gray-400 ${numericFontClass}`}>
          ...
          <div className="bg-black border border-gray-800 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {nftData.map((nft, idx) => (
                <div
                  key={idx}
                  className="bg-gray-900/50 border border-gray-800 hover:border-[#ccff00]/30 transition-all group"
                >
            ...
            {protocolData.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {protocolData.map((protocol, idx) => (
                  <div
                    key={idx}
                    className="bg-black border border-gray-800 p-6 hover:border-[#ccff00]/30 transition-all group"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {protocol.supplied !== undefined && protocol.supplied > 0 && (
                        <div>
                          <div
                            className={`text-xs text-gray-500 uppercase tracking-wider font-bold mb-1 ${headingFontClass}`}
                          >
                            Supplied
                          </div>
                ...
                <tbody className="divide-y divide-gray-800">
                  {txData.map((tx, idx) => (
                    <tr key={idx} className="hover:bg-gray-900/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className={`text-sm font-medium text-white ${numericFontClass}`}>
                          {tx.fromAmount.toFixed(4)} ETH
                        </div>

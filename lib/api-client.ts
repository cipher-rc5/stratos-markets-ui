...
class StratosApiClient {
  ...
  strategies = {
    ...
    subscribe: async (id: string, walletAddress: string) => {
      return this.request<any>(`/strategies/${id}/subscribe`, {
        method: "POST",
    ...
    unsubscribe: async (id: string, walletAddress: string) => {
      return this.request<any>(
        `/strategies/${id}/subscribe?walletAddress=${walletAddress}`,
        {
          method: "DELETE",
        }
      )
  ...
  agents = {
    ...
    execute: async (id: string, walletAddress: string, parameters?: any) => {
      return this.request<any>(`/agents/${id}/execute`, {
        method: "POST",
        body: JSON.stringify({ walletAddress, parameters }),
      })
    },
  ...
  portfolio = {
    ...
    getHistory: async (walletAddress: string, timeframe = "30d") => {
      return this.request<any[]>(
        `/portfolio/history?walletAddress=${walletAddress}&timeframe=${timeframe}`
      )
    },
  ...
  market = {
    ...
    getChart: async (symbol: string, interval = "1h", limit = 100) => {
      return this.request<any[]>(
        `/market/${symbol}/chart?interval=${interval}&limit=${limit}`
      )
    },
  }
}
